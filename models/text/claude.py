...
async def claude_chat(messages, parameters):
    ...
    with open("models/text/claude_config.json", "r") as f:
        claude_configs = json.load(f)
    
    primary_config = claude_configs[0]["litellm_params"]
    fallback_configs = [config["litellm_params"] for config in claude_configs[1:]]
