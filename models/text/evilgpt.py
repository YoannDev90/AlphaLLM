import logging
from utils.config import logger_name, API_ENDPOINTS_TEXT, MODELS_CONFIG_TEXT
...
async def evilgpt_chat(messages, parameters):
    start_time = datetime.now()
    opik_logger = OpikLogger()
    litellm.callbacks = [opik_logger]

    # Load EvilGPT configurations
    with open("models/text/evilgpt_config.json", "r") as f:
        evilgpt_configs = json.load(f)
    
    primary_config = evilgpt_configs[0]["litellm_params"]
    fallback_configs = [config["litellm_params"] for config in evilgpt_configs[1:]]
