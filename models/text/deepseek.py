...
async def deepseek_chat(messages, parameters):
    ...
    with open("models/text/deepseek_config.json", "r") as f:
        deepseek_configs = json.load(f)

    primary_config = deepseek_configs[0]["litellm_params"]
    fallback_configs = [config["litellm_params"] for config in deepseek_configs[1:]]
    ...
    try:
        ...
        return handle_litellm_error(e, model="deepseek")
    except Exception as e:
        logger.error(f"Error with DeepSeek: {e}")
        return {
            "response": f"Error with DeepSeek: {e}",
            "usage": 0,
            "model": "error",
