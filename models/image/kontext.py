...
async def generate_kontext(prompt: str, size: str = "1024x1024", images_url : list[str] = None) -> str:
    """
    ...
    if images_url:
        logger.info(f"Adding {len(images_url)} images for kontext: {images_url}")
...
if __name__ == "__main__":
    prompt = input("Enter prompt (default: 'A mystical forest with glowing trees'): ").strip() or "A mystical forest with glowing trees"
    ...
    try:
        result = asyncio.run(generate_kontext(prompt, size))
        ...
        image_filename = output_dir / "kontext_output.png"
