...
async def generate_nanobanana(
    ...
    """
    Génère une image avec le modèle Nanobanana via Pollinations
    ...
    if images_url:
        logger.info(f"Adding {len(images_url)} images for nanobanana: {images_url}")
...
if __name__ == "__main__":
    prompt = (
        input("Enter prompt (default: 'A vibrant coral reef with fish'): ").strip()
        or "A vibrant coral reef with fish"
    )
