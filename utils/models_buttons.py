...
import discord
from utils.roles_utils import save_role_to_db, get_role_from_db, create_role, assign_role_to_bot, remove_role_from_bot, delete_role, remove_role_from_db
import json
    ...
    def update_button_status(self):
        ...
        if role_id:
            ...
            if role and role in self.guild.me.roles:
                ...
                logger.warning(f"Le rôle {self.model['role_name']} n'existe pas.")
                remove_role_from_db(role_id[0])
                self.style = discord.ButtonStyle.success
            elif role not in self.guild.me.roles:
                logger.warning(f"Le bot n'a pas le rôle {role.name}.")
                self.style = discord.ButtonStyle.blurple
            else:
                logger.error(f"Erreur inconnue pour le rôle {role.name}.")
        else:
            self.style = discord.ButtonStyle.success
