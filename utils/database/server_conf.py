...
async def add_to_allowed_channels(server_id: int, channel_id: int):
    try:
        rows = await db_manager.exc_get_query("SELECT allowed_channels FROM server_settings_new WHERE server_id = ?", (server_id,))
        if rows:
...
async def add_to_allowed_roles(server_id: int, role_id: int):
    try:
        rows = await db_manager.exc_get_query("SELECT allowed_roles FROM server_settings_new WHERE server_id = ?", (server_id,))
        if rows:
            allowed = json.loads(rows[0][0]) if rows[0][0] else []
...
async def set_language(server_id: int, language_code: str):
    try:
        rows = await db_manager.exc_get_query("SELECT server_id FROM server_settings_new WHERE server_id = ?", (server_id,))
        if rows:
            await db_manager.exc_modify_query(
...
async def set_allowed_channels(server_id: int, channel_ids: list[int]):
    try:
        ...
        logger.info(f"Canaux autorisés mis à jour pour le serveur {server_id}: {channel_ids}")
    except Exception as e:
...
async def set_allowed_roles(server_id: int, role_ids: list[int]):
    try:
        ...
        logger.info(f"Rôles autorisés mis à jour pour le serveur {server_id}: {role_ids}")
    except Exception as e:
