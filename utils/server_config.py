import json
import logging
from utils.config import LOGGER_NAME, DEBUG
...
async def update_all_guilds_info(bot):
    ...
    # Récupérer tous les serveurs présents dans la base de données
    try:
    ...
    # Récupérer les IDs des serveurs actuels du bot
    current_guild_ids = {guild.id for guild in bot.guilds}
    
    # Mettre à jour/créer les serveurs actuels
    for guild in bot.guilds:
        try:
            ...
            # Récupérer le nom du propriétaire de manière plus robuste
            if guild.owner:
                owner_name = guild.owner.name  # Utiliser .name au lieu de .display_name
            elif guild.owner_id:
                # Si on n'a pas l'objet owner mais qu'on a l'ID, essayer de le récupérer
                try:
    ...
    # Supprimer les serveurs qui ne sont plus dans la liste du bot
