...
_launcher = CONFIG.get("launcher", {})
LAUNCHER_RUN_API = _launcher.get("run_api", True)
LAUNCHER_RUN_MAIN_BOT = _launcher.get("run_main_bot", True)
LAUNCHER_RUN_SECONDARY_BOTS = _launcher.get("run_secondary_bots", True)
LAUNCHER_RUN_ADMIN_BOT = _launcher.get("run_admin_bot", True)
LAUNCHER_RUN_LOGGER_BOT = _launcher.get("run_logger_bot", True)
...
OPEN_TELEMETRY_URL = _links.get("open_telemetry", "https://cloud.langfuse.com/api/public/otel")
...
def update_log_level(new_level: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Erreur lors de la modification du fichier config.toml: {e}")
        return False
...
def get_current_log_level() -> Optional[str]:
    try:
        ...
        logger.error(f"Erreur lors de la lecture du niveau de log: {e}")
        return None
...
def get_logs_channel_config() -> Dict[str, Any]:
    """Récupère la configuration du salon logs from config.toml"""
    try:
        config = load_toml_config("config.toml")
        return config.get("logs_channel", {})
    except Exception as e:
        logger.error(f"Erreur lors de la lecture de la configuration du salon logs: {e}")
        return {}

def update_logs_channel_config(channel_id: int = None, channel_name: str = None, 
                               position: int = None, overwrites: Dict = None) -> bool:
    """Met à jour la configuration du salon logs dans config.toml"""
    config_path = "config.toml"
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            elif logs_channel_start is not None and line.startswith("[") and line.strip() != "[logs_channel]":
                logs_channel_end = i
                break
        ...
        if channel_id is not None:
            new_section.append(f"channel_id = {channel_id}")
        ...
        logger.error(f"Erreur lors de la mise à jour du salon logs dans config.toml: {e}")
        return False