...
async def unified_text_gen(
    ...
    """
    Fonction pour gérer les requêtes vers les modèles Discord.
    """
    logger.debug(
        f"Entered unified_text_gen with user_id={user_id}, conv_id={conv_id}, input='{input}', model={model}"
    )
    ...
    if tool_calls:
        ...
        for call in tool_calls:
            ...
            from utils.func_calling.tools import execute_tool
    ...
    user_name = (
        user.display_name if user else f"User_{user_id}"
    )
