...
def update_status_on_success(model: str, elapsed_time: str):
    ...
    entry['success_rate'] = (entry['successful_requests'] / entry['total_requests']) * 100
    try:
...
def update_status_on_failure(model: str):
    ...
    entry['total_requests'] += 1
    entry['last_check'] = time.time()
    entry['success_rate'] = (entry['successful_requests'] / entry['total_requests']) * 100 if entry['total_requests'] > 0 else 0.0
        ...
        for model in AVAILABLE_MODELS:
            try:
                results = [r async for r in unified_text_gen(
                    user_id=0, conv_id="status_check", input="test", model=model,
