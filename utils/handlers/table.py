    ...
    try:
        ...
        for i, line in enumerate(lines):
            if line.startswith("|") and line.endswith("|"):
                if re.match(r"^\|[\s\-\:\|]*\|$", line):
                    ...
                    continue
                cells = [
                    clean_cell_content(cell.strip()) for cell in line[1:-1].split("|")
                ]
                rows.append(cells)
        ...
        for row in rows:
            for idx, cell in enumerate(row):
                ...
                max_line_width = (
                    max(len(line) for line in cell_lines) if cell_lines else 0
                )
        ...
        if total_natural_width > max_line_length:
            ...
            if total_content_width > 0:
                ...
                col_widths = [
                    max(1, int(width * ratio)) for width in natural_col_widths
                ]
            else:
        ...
        if rows:
            ...
            for idx, cell in enumerate(header_cells):
                ...
                wrapped_headers.append(
                    textwrap.wrap(cell.upper(), width=width) or [cell.upper()]
                )
            max_wrap = max(len(w) for w in wrapped_headers)
            for i in range(max_wrap):
                row_cells = [
                    wrapped_headers[idx][i] if i < len(wrapped_headers[idx]) else ""
        ...
        for row in rows[1:]:
            ...
            for i in range(max_wrap):
                row_cells = [
                    wrapped_cells[idx][i] if i < len(wrapped_cells[idx]) else ""
                    for idx in range(len(col_widths))
                ]
                row_text = (
                    "|"
