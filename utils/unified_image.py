...
def validate_image_size(model: str, size: str) -> bool:
    ...
    if size in sizes:
        ...
        logger.warning(
            f"Requested size {size} not supported by model {model}. Supported sizes: {sizes}"
        )
...
async def _generate_with_fallbacks(
    prompt: str, size: str, primary_model: str
) -> Optional[str]:
        ...
        if model in IMAGE_GEN_FUNCTIONS:
            try:
                ...
                if result is not None:
                    ...
                    logger.warning(
                        f"Model {model} returned None, trying next fallback."
                    )
            except Exception as e:
...
async def unified_image_edit(
    ...
    num_images = min(max(1, num_images), 4)
    file_bytes = BytesIO(
        await (await aiohttp.ClientSession().get(images_url[0])).read()
    )
...
async def _edit_with_fallbacks(
    prompt: str, size: str, images_url: list[str], primary_model: str
) -> Optional[str]:
    """Tente de générer une image avec le modèle primaire, puis les fallbacks."""
