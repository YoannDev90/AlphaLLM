...
from utils.database.client import get_blacklist, get_allowed_channels, get_allowed_roles
from utils.processing.ai_handler import process_ai_response
from utils.database.user_manager import new_interaction, new_query
...
async def smart_long_messages_with_view(channel, response, original_question, model, response_data, bot, search_internet=False, max_length: int = 2000):
    """
    Sends a long message to a Discord channel with MessageView buttons, preserving code blocks and never splitting inside a code block.
    This is a wrapper for use with discord.Message channels instead of interactions.
    """
    import re
    from embeds.message import MessageView
    ...
    for part in parts:
        if part.startswith("```") and part.endswith("```"):
            ...
            code_prefix = "```" if not code_lines[0].startswith("```") else code_lines[0] + "\n"
            code_suffix = "```"
            current_code = code_prefix
            
            for line in code_lines[1:-1] if code_lines[0].startswith("```") else code_lines:
                if len(current_code) + len(line) + len(code_suffix) > max_length:
                    current_code += code_suffix
    ...
    # Add view to last message if available
    if last_message:
        view = MessageView(original_question, model, response_data, bot, search_internet)
        await last_message.edit(view=view)
        view.message = last_message