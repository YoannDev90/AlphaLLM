...
async def setup(bot: discord.Client):
    @bot.tree.command(name="guild-lang", description="Define guild language")
    ...
    async def langue(interaction: discord.Interaction, langue: app_commands.Choice[str]):
        ...
        logger.info(f"Commande guild-lang exécutée par {interaction.user.display_name}")
        ...
        try:
            ...
            logger.info(f"Langue mise à jour pour {interaction.guild.name} : {langue.value}")

        except Exception as e:
            logger.error(f"Erreur mise à jour langue : {str(e)}")
            await interaction.followup.send("❌" + tlt(language=user_lang, key="lang_settings_error"), ephemeral=True)