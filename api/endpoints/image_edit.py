...
async def edit_image(
    ...
    if model not in image_models:
        raise HTTPException(
            status_code=400,
            detail=f"Model '{model}' is not available for image editing",
        )

    logger.info(
        f"Image edit request: user_id={user_id}, model={model}, num_images={num_images}, enhance={enhance}, prompt={prompt[:50]}..."
    )
