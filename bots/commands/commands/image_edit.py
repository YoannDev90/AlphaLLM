...
async def setup(bot: discord.Client):
    ...
    async def image_edit(
        ...
        logger.info(f"Début de l'envoi de {len(results)} image(s) pour {interaction.user.display_name}")
        
        if warning_message:
            await interaction.followup.send(warning_message.rstrip())
        
        for idx, (image_data, current_prompt) in enumerate(results, start=1):
            logger.info(f"Traitement de l'image {idx}/{number} - Données présentes: {image_data is not None}")
            if image_data:
                try:
