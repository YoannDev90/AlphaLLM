    ...
    async def guilds(
        ...
        try:
            ...
            sorted_guilds = sorted(
                guilds,
                key=lambda guild: (
                    guild.me.joined_at if guild.me else discord.utils.utcnow()
                ),
                reverse=True,
            )
            ...
            total_members = sum(
                guild.member_count for guild in guilds if guild.member_count
            )
            ...
            stats_embed.add_field(
                name="🏢 Serveurs", value=f"**{total_guilds}** serveurs", inline=True
            )

            stats_embed.add_field(
                name="👥 Membres", value=f"**{total_members:,}** membres", inline=True
            )
            ...
            match data.value:
                ...
                case 1:
                    ...
                    for i, guild in enumerate(latest_guilds, start=1):
                        joined_date = (
                            guild.me.joined_at.strftime("%d/%m/%Y")
                ...
                case 2:
                    ...
                    total_messages = (
                        total_guilds + servers_per_message - 1
                    ) // servers_per_message
            ...
            logger.error(
                f"Erreur Forbidden lors de l'exécution de la commande guilds: {e}"
            )
            await interaction.followup.send(
                "Le bot n'a pas la permission de récupérer les informations demandées.",
                ephemeral=True,
            )
        except Exception as e:
            logger.error(
                f"Erreur inattendue lors de l'exécution de la commande guilds: {e}"
            )
            await interaction.followup.send(
