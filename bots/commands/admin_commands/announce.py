...
from utils.database.server_conf import get_announcement_channel
...
async def find_announcement_channel(guild):
    ...
    announce_channel_id = await get_announcement_channel(guild.id)
    target_channel = None
    ask_define = False
    ...
    if announce_channel_id:
        ...
        logger.info(
            f"Configured announcement channel found: {target_channel} (ID: {announce_channel_id})"
        )
        ask_define = False
...
async def send_announcement_to_guilds(bot, message: str) -> tuple[int, int]:
    ...
    for guild in bot.guilds:
        ...
        try:
            ...
            if not target_channel:
                logger.warning(
                    f"No announcement channel found for server {guild.name} (ID: {guild.id})"
                )
                failed_guilds += 1
                continue
            ...
            logger.error(
                f"Insufficient permissions to send message to server {guild.name} (ID: {guild.id})"
            )
            failed_guilds += 1
    ...
    logger.info(
        f"Announcement complete: {announced_guilds} successful, {failed_guilds} failed"
    )
...
async def send_channel_config_suggestion(target_channel, guild):
    ...
    logger.info(
        f"Requesting announcement channel configuration for {guild.name} (ID: {guild.id})"
    )
    try:
...
async def setup(bot: discord.Client):
    @bot.tree.command(
        name="announce", description="Send an announcement to all servers"
    )
