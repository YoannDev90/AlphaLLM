...
async def setup(bot: discord.Client):
    @bot.tree.command(name="whitelist_all", description="Whitelist tous les salons et rôles de tous les serveurs")
    async def whitelist_all(interaction: discord.Interaction):
        ...
        for guild in main_bot.guilds:
            try:
                ...
                updated_guilds += 1
                logger.info(f"Whitelist mis à jour pour le serveur {guild.name} ({guild.id})")

            except Exception as e:
                logger.error(f"Erreur lors de la mise à jour du serveur {guild.name}: {e}")

        await interaction.followup.send(f"Whitelist mis à jour pour {updated_guilds}/{total_guilds} serveurs.", ephemeral=True)