        ...
        try:
            with open(POLL_DATA_FILE, "r") as f:
                ...
                for poll_id, poll_data in data.items():
                    for key in ("end_time", "created_at"):
                        if key in poll_data and poll_data[key]:
                            poll_data[key] = datetime.fromisoformat(poll_data[key])
...
class VoteButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if (
            poll_data.get("end_time")
            and datetime.now(timezone.utc) > poll_data["end_time"]
        ):
            await interaction.response.send_message(
...
async def update_poll_message(client, poll_message_id: int):
    ...
    try:
        ...
        if poll_data.get("end_time"):
            if datetime.now(timezone.utc) > poll_data["end_time"]:
...
async def setup(tree: app_commands.CommandTree, bot: discord.Client):
    ...
    @tree.command(name="poll", description="Create a poll for users to vote on")
