...
async def setup(bot: discord.Client):
    ...
    async def leave(
        ...
        logger.info(f"Commande /leave exécutée par {interaction.user.display_name} pour le serveur {guild_id}")
        ...
        try:
            await guild.leave()
            await interaction.followup.send(f"Le bot a quitté le serveur **{guild.name}** (ID: `{guild_id}`).", ephemeral=True)
        except discord.HTTPException as e:
            logger.error(f"Erreur HTTP lors de la tentative de quitter le serveur {guild_id}: {e}")
            await interaction.followup.send("Erreur lors de la tentative de quitter le serveur. Veuillez réessayer plus tard.", ephemeral=True)
        except Exception as e:
            logger.error(f"Erreur inattendue lors de la tentative de quitter le serveur {guild_id}: {e}")
